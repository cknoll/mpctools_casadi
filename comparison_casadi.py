...
ode_integrator = casadi.SXFunction(casadi.daeIn(x=x,p=u),casadi.daeOut(ode=casadi.vertcat(ode(x,u))))
...
ode_casadi = casadi.SXFunction([x,u],[casadi.vertcat(ode(x,u))])
ode_casadi.init()
...
nlp = casadi.SXFunction(casadi.nlpIn(x=var),casadi.nlpOut(f=obj,g=con))
