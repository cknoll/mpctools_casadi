...
\funcname{util.rk4(f,x0,par,Delta,M=1)}

Returns an explicit Runge-Kutta 4th order discretization with \texttt{M} steps of size \texttt{Delta}/\texttt{M}.
\texttt{x0} should be the initial condition for the differential variables, and \texttt{par} should be a list of all other inputs to \texttt{f} (which are held constant during integration).
...
def ode_rk4(x,u):
    return mpc.util.rk4(ode,x,[u],Delta)

ode_rk4_casadi = mpc.getCasadiFunc(
    ode_casadi, [2,1], ["x","u"])
\end{lstlisting}
